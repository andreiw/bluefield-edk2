...
{
  RETURN_STATUS Status0, Status1;

  Status0 = PL011UartInitializePort (
        BLUEFIELD_UART0_BASE,
        BaudRate,
        ReceiveFifoDepth,
        Parity,
        DataBits,
        StopBits);
  Status1 = PL011UartInitializePort (
        BLUEFIELD_UART1_BASE,
        BaudRate,
        ReceiveFifoDepth,
        Parity,
        DataBits,
  ...
  // Return SUCCESS if any of them succeeds, or else just return Status0.
  ...
  )
{
  ...
  Status = PL011UartSetControl (BLUEFIELD_UART0_BASE, Control);
  if (Status == RETURN_SUCCESS) {
    Status = PL011UartSetControl (BLUEFIELD_UART1_BASE, Control);
  }
  ...
  )
{
  ...
  // Check TmFifo console input.
  if (TmFifoConsPoll()) {
    *Control &= ~EFI_SERIAL_INPUT_BUFFER_EMPTY;
    return RETURN_SUCCESS;
  }

  // Round-robin check the two UART.
  if (Idx++ & 1) {
    return PL011UartGetControl(BLUEFIELD_UART0_BASE, Control);
  } else {
    return PL011UartGetControl(BLUEFIELD_UART1_BASE, Control);
  }
}
